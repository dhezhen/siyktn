<x-layouts::app :title="'Dashboard'">
    ...
    @isset($pimpinanData)
        <div class="mt-8 mb-4">
            <h2 class="text-xl font-bold text-slate-800 dark:text-slate-100">Ringkasan Eksekutif (Pimpinan)</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">Laporan keuangan, demografi, dan beban operasional instansi secara keseluruhan.</p>
        </div>
        
        @include('pimpinan.partials.widgets', $pimpinanData)
    @endisset
    ...
    <div class="mt-6 grid gap-4 lg:grid-cols-3">
        ...
        @can('activity.view')
            <x-card class="lg:col-span-2" title="Aktivitas Terakhir" subtitle="10 perubahan data paling baru.">
                @forelse ($activities ?? [] as $activity)
                    <div class="flex items-start gap-3 border-b border-slate-100 dark:border-slate-800 py-2.5 last:border-0">
        ...
        <x-card :class="auth()->user()->can('activity.view') ? '' : 'lg:col-span-3'" title="Selamat Datang">
            <p class="text-sm leading-relaxed text-slate-600 dark:text-slate-400">
                Halo <span class="font-medium text-slate-900 dark:text-slate-200">{{ auth()->user()->name }}</span>,
                Anda masuk sebagai
                <span class="font-medium text-slate-900 dark:text-slate-200">{{ auth()->user()->roles->pluck('name')->join(', ') ?: 'pengguna tanpa role' }}</span>.
            </p>
